...
CREATE TABLE committee_decisions(
    ...
    user_id NUMERIC (50) REFERENCES users(user_id),
    committee_name VARCHAR (50) REFERENCES committee(committee_name),
...
CREATE TABLE good_feedback (
    user_id_given_feedback NUMERIC ( 50 ) REFERENCES users(user_id),
    user_id NUMERIC ( 50 ) REFERENCES users(user_id),
...
CREATE TABLE inbox (
   inbox_id SERIAL PRIMARY KEY NOT NULL,
    receiver NUMERIC ( 50 ) REFERENCES users(user_id) NOT NULL,
    sender NUMERIC ( 50 ) REFERENCES users(user_id) NOT NULL,
    subject VARCHAR ( 50 )  ,
    content VARCHAR ( 50 ) ,
    time TIMESTAMP
    );
 ...
 CREATE TABLE usage_data(
    user_id NUMERIC ( 50 ) REFERENCES users(user_id) NOT NULL,
...
CREATE TABLE xpertesy(
    ...
    host_id NUMERIC (50) REFERENCES users(user_id) NOT NULL,
