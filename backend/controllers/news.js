...
const eduUrls = [
  'https://edu.gov.il/owlHeb/Pages/NewsAgg.aspx',
...
const knessetUrl =
  'https://main.knesset.gov.il/News/PressReleases/Pages/default.aspx';
...
exports.scrapeNews = async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < eduUrls.length; i++) {
      // scrape edu.gov
      ...
      const newsOnjList = $('.secondary .container', html)
        .map((i, e) => {
          return {
            name: $(e).find('h2').text(),
    ...
    const newsOnjList = $(
      '#KnessetNewsLobbyMainContainerDiv > div > div > div.main-container-left-side > div.item.news-content > ul li',
      html
    )
      .map((i, e) => {
        return {
